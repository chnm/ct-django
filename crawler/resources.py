...
class StagedMuseumItemResource(resources.ModelResource):
    ...
    reviewed_by = fields.Field(attribute="reviewed_by__username", column_name="Reviewed By")
    
    # Publishing Status
    published = fields.Field(attribute="published", column_name="Published")
    published_to_id = fields.Field(attribute="published_to__id", column_name="Published To Record ID")
    
    # Timestamps
    initial_date_fetched = fields.Field(attribute="initial_date_fetched", column_name="Date Fetched")
    ...
    def dehydrate_api_data_summary(self, obj):
        ...
        try:
            ...
            # Common fields
            if 'participants' in api_data and api_data['participants']:
                participants = api_data['participants']
                if participants:
                    participant_names = []
                    for participant in participants:
                        if isinstance(participant, dict) and 'person_name' in participant:
